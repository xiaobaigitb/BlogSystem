...
public class BloggerPageController {
    ...
    public ModelAndView mainPage(HttpServletRequest request,
        ...
        System.out.println("---------------"+ownerId+"--------"+mv.toString());
        BloggerProfile profile = bloggerProfileService.getBloggerProfile(ownerId);
        System.out.println(profile.getIntro()+"----"+profile.getAboutMe()+"----"+profile.getAvatarId());
        mv.addObject("blogName", profile.getIntro());
        mv.addObject("aboutMe", profile.getAboutMe());
        //mv.addObject("avatarId",profile.getAvatarId());
        //todo bug页面显示不出来,因为找不到默认图片---已经修复（博主的图片类型ID要指定成13）
